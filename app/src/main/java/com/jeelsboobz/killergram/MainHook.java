        ...
        if (hookPackages.contains(lpparam.packageName)) {
            try {
                ...
                if (lpparam.packageName.equals("ir.ilmili.telegraph")) {
                    ...
                    Class<?> MessagesControllerTelegraph = XposedHelpers.findClassIfExists("org.telegram.messenger.m80", lpparam.classLoader);
                    if (MessagesControllerTelegraph != null) {
                        XposedBridge.hookAllMethods(MessagesControllerTelegraph, "q9", XC_MethodReplacement.returnConstant(false));
                    }
                    Class<?> UserConfigTelegraph = XposedHelpers.findClassIfExists("org.telegram.messenger.cu0", lpparam.classLoader);
                    if (UserConfigTelegraph != null) {
                        XposedBridge.hookAllMethods(UserConfigTelegraph, "L", XC_MethodReplacement.returnConstant(true));
                    }
                }
