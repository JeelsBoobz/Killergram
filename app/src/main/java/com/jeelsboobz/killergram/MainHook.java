        ...
        if (hookPackages.contains(lpparam.packageName)) {
            try {
                ...
                if (lpparam.packageName.equals("ir.ilmili.telegraph")) {
                    ...
                    Class<?> MessagesControllerTelegraph = XposedHelpers.findClassIfExists("org.telegram.messenger.ha0", lpparam.classLoader);
                    if (MessagesControllerTelegraph != null) {
                        XposedBridge.hookAllMethods(MessagesControllerTelegraph, "P9", XC_MethodReplacement.returnConstant(false));
                    }
                    Class<?> MessagesControllerTelegraph2 = XposedHelpers.findClassIfExists("org.telegram.messenger.uu", lpparam.classLoader);
                    if (MessagesControllerTelegraph2 != null) {
                        XposedBridge.hookAllMethods(MessagesControllerTelegraph2, "B", XC_MethodReplacement.returnConstant(false));
                    }
                    Class<?> UserConfigTelegraph = XposedHelpers.findClassIfExists("org.telegram.messenger.cw0", lpparam.classLoader);
